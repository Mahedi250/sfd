...
function ServiceCard({ tone, IconComp, title, desc, learnMore }) {
  ...
  return (
    <div
      className="svc-card"
      style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 20, padding: "24px 24px 20px", position: "relative", transition: "transform .22s, box-shadow .22s, border-color .22s", cursor: "pointer", overflow: "hidden", display: "flex", flexDirection: "column", gap: 0 }}
      onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-5px)"; e.currentTarget.style.boxShadow = "0 12px 32px rgba(0,0,0,.09)"; e.currentTarget.style.borderColor = bar; }}
    ...
    >
      ...
      <h3 style={{ fontSize: 17, fontWeight: 700, margin: "0 0 6px", letterSpacing: "-0.015em", lineHeight: 1.2 }} className="lang-swap">{title}</h3>
      <p style={{ margin: "0 0 18px", color: "var(--ink-3)", fontSize: 13.5, lineHeight: 1.55, flex: 1 }} className="lang-swap">{desc}</p>
